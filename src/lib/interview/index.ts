...
export function getQuestion(
  ...
  try {
    ...
    console.error('CRITICAL ERROR in getQuestion:', e);
    // Fallback to a basic question
    const fallbackQuestion = {
      question: 'Tell me about a recent project you worked on and your role in it.',
      category,
      difficulty,
      role,
      topics: [],
      expectedConcepts: ['project', 'role', 'responsibilities'],
      hints: ['Be specific about your contributions', 'Mention technologies used', 'Describe the outcome']
    };
    const session = createSession(role, difficulty);
    return {
      success: true,
...
export function submitAnswer(
  ...
  try {
    ...
    // Get role knowledge for topics - with defensive checks
    const roleKnowledge = roleKnowledgeBase[role];
    if (!roleKnowledge) {
      console.error(`Unknown role: ${role}. Falling back to software-developer.`);
      role = 'software-developer';
    }
    
    const safeRoleKnowledge = roleKnowledgeBase[role] || roleKnowledgeBase['software-developer'];
    const topics = safeRoleKnowledge?.topics?.[category] || safeRoleKnowledge?.topics?.technical || [];
    ...
    console.error('CRITICAL ERROR in submitAnswer:', e);
    return {
      success: false,
...
export function getFollowUpQuestion(
  ...
  try {
    ...
    console.error('CRITICAL ERROR in getFollowUpQuestion:', e);
    const session = createSession(role, difficulty);
    const fallbackQuestion = {
      question: 'Can you elaborate more on your previous answer?',
      category,
      difficulty,
      role,
      topics: [],
      expectedConcepts: ['detail', 'elaboration'],
      hints: ['Provide more specific details', 'Use examples to illustrate your points']
    };
    return {
      success: true,
      question: fallbackQuestion,
...
export function getInterviewSummary(sessionId: string): {
  ...
  try {
    ...
    const roleKnowledge = roleKnowledgeBase[session.role] || roleKnowledgeBase['software-developer'];
    ...
    return {
      ...
      performance,
      strengths: roleKnowledge?.keySkills?.slice(0, 3) || ['problem-solving', 'communication', 'technical skills'],
      areasToImprove: roleKnowledge?.topics?.technical?.[0]?.concepts?.slice(0, 2) || ['technical depth', 'structured thinking'],
      duration
    };
  } catch (e) {
    console.error('CRITICAL ERROR in getInterviewSummary:', e);
    return null;
  }
}
