...
const categories = [
  ...
  },
  {
    ...
    borderHover: 'hover:border-green-300 dark:hover:border-green-500/50',
    stats: '2+ tools',
  },
  {
    title: 'Productivity',
  ...
  },
  {
    ...
    lightGradient: 'from-indigo-50 to-indigo-100/50',
...
export function CategoriesSection() {
  return (
    <section className="py-24 bg-slate-50 dark:bg-slate-950 relative overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-purple-100/70 dark:from-purple-900/5 via-transparent to-transparent" />
          ...
          {categories.map((category) => (
            <motion.div key={category.title} variants={itemVariants}>
              <Link href={category.href}>
                <div className={`group relative p-6 rounded-2xl bg-gradient-to-br ${category.lightGradient} dark:bg-gradient-to-br dark:${category.gradient} border border-slate-200 dark:border-slate-800/50 ${category.borderHover} transition-all duration-300 hover:shadow-xl hover:-translate-y-1 h-full`}>
                  {/* Icon */}
                  ...
                  {/* Content */}
                  <h3 className="font-semibold text-lg mb-2 text-slate-900 dark:text-white group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">
                    {category.title}
                  </h3>
                  <p className="text-sm text-slate-600 dark:text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-300 transition-colors mb-4">
                    {category.description}
                  </p>

                  {/* Stats & CTA */}
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-500 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-400">
                      {category.stats}
                    </span>
